    ...
    if (count($input_categories)) {
        foreach ($input_categories as $key => $category) {
            if ($category['parent_id'] == $parent_id) {
                ...
                outputLevelCategories($input_categories, $output_categories, $new_parent_id, $new_lvl);
            }
        }
    }
...
foreach ($result as $item) {
    echo '<br> id : ' . $item['id'] . ' - ' . str_repeat('_', $item['level'] - 1) . $item['name'];
}
